...
class PackWalker extends IgnoreWalker {
  ...
  filterEntries () {
    if (this.ignoreRules['package.json']) {
      ...
      process.emitWarning(
        `No .npmignore file found. Falling back to .gitignore for file exclusion in ${this.path}. ` +
        'Consider creating a .npmignore file to explicitly control which files are published.',
        {
          type: 'Warning',
          code: 'NPMPACK_GITIGNORE_FALLBACK',
        }
      )
    }
